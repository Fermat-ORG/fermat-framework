...
public class AssetIssuingTransactionDao {
    ...
    public IssuingStatus getIssuingStatusByAssetPublicKey(String assetPublicKey) throws CantExecuteDatabaseOperationException {
        String context = "Asset Public Key: " + assetPublicKey;
        if (assetPublicKey == null) {
            throw new CantExecuteDatabaseOperationException(null, context, "The given asset public key is null.");
        }
        try {
            this.database = openDatabase();
            DatabaseTable databaseTable = getDatabaseTable(AssetIssuingTransactionDatabaseConstants.DIGITAL_ASSET_TRANSACTION_TABLE_NAME);
            ...
            if (databaseTableRecords.isEmpty()) {
                throw new CantExecuteDatabaseOperationException(null, context, "The given asset public key is not registered in the database.");
            }

            return IssuingStatus.getByCode(databaseTableRecords.get(0).getStringValue(AssetIssuingTransactionDatabaseConstants.DIGITAL_ASSET_TRANSACTION_DIGITAL_ASSET_ISSUING_STATUS_COLUMN_NAME));
        } catch (CantExecuteDatabaseOperationException exception) {
            throw new CantExecuteDatabaseOperationException(exception, context, "Cannot find or open the database");
        } catch (CantLoadTableToMemoryException exception) {
            throw new CantExecuteDatabaseOperationException(exception, context, "Cannot load the database into memory");
        } catch (InvalidParameterException exception) {
            throw new CantExecuteDatabaseOperationException(exception, context, "The saved parameter is invalid, check nested exception.");
        } catch (Exception exception) {
            throw new CantExecuteDatabaseOperationException(FermatException.wrapException(exception), context, "Unexpected exception");
        } finally {
            database.closeDatabase();
        }
    }
