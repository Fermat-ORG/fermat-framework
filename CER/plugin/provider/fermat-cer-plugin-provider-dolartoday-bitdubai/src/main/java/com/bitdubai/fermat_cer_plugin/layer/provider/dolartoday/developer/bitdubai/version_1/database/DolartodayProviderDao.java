...
public class DolartodayProviderDao {
    ...
    public void initialize() throws CantInitializeDolarTodayProviderDatabaseException {
        try {
            ...
            DollarTodayProviderDatabaseFactory databaseFactory = new DollarTodayProviderDatabaseFactory(pluginDatabaseSystem);
            try {
                ...
                throw new CantInitializeDolarTodayProviderDatabaseException("Database could not be opened", cantCreateDatabaseException, "Database Name: " + DollarTodayProviderDatabaseConstants.QUERY_HISTORY_TABLE_NAME, "");
            }
            ...
            throw new CantInitializeDolarTodayProviderDatabaseException("Database could not be opened", cantOpenDatabaseException, "Database Name: " + DollarTodayProviderDatabaseConstants.QUERY_HISTORY_TABLE_NAME, "");
        } catch (Exception e) {
            ...
            throw new CantInitializeDolarTodayProviderDatabaseException("Database could not be opened", FermatException.wrapException(e), "Database Name: " + DollarTodayProviderDatabaseConstants.QUERY_HISTORY_TABLE_NAME, "");
        }
    }
    ...
    public void saveExchangeRate(ExchangeRate exchangeRate) throws CantSaveExchangeRateException {
        ...
        DatabaseTable table = this.database.getTable(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_TABLE_NAME);
    ...
    private DatabaseTableFilter getEmptyHoldTableFilter() {
        return this.database.getTable(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_TABLE_NAME).getEmptyTableFilter();
    }
    ...
    private void constructRecordFromExchangeRate(DatabaseTableRecord newRecord, ExchangeRate exchangeRate) {
        ...
        newRecord.setUUIDValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_ID_COLUMN_NAME, UUID.randomUUID());
        newRecord.setStringValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_FROM_CURRENCY_COLUMN_NAME, exchangeRate.getFromCurrency().getCode());
        newRecord.setStringValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_TO_CURRENCY_COLUMN_NAME, exchangeRate.getToCurrency().getCode());
        newRecord.setDoubleValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_SALE_PRICE_COLUMN_NAME, exchangeRate.getSalePrice());
        newRecord.setDoubleValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_PURCHASE_PRICE_COLUMN_NAME, exchangeRate.getPurchasePrice());
        newRecord.setLongValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_TIMESTAMP_COLUMN_NAME, (new Date().getTime() / 1000));

    }

    private ExchangeRate constructExchangeRateFromRecord(DatabaseTableRecord record) throws CantCreateExchangeRateException {

        UUID id = record.getUUIDValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_ID_COLUMN_NAME);
        double salePrice = record.getDoubleValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_SALE_PRICE_COLUMN_NAME);
        double purchasePrice = record.getDoubleValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_PURCHASE_PRICE_COLUMN_NAME);
        long timestamp = record.getLongValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_TIMESTAMP_COLUMN_NAME);

        Currency fromCurrency;
        try {
            fromCurrency = Currency.getByCode(record.getStringValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_FROM_CURRENCY_COLUMN_NAME));
            ...
            throw new CantCreateExchangeRateException(e.getMessage(), e, "Dolartoday provider plugin", "Invalid From Currency value stored in table"
                    + DollarTodayProviderDatabaseConstants.QUERY_HISTORY_TABLE_NAME + " for id " + id);
        }

        Currency toCurrency;
        try {
            toCurrency = Currency.getByCode(record.getStringValue(DollarTodayProviderDatabaseConstants.QUERY_HISTORY_TO_CURRENCY_COLUMN_NAME));
        } catch (InvalidParameterException e) {
