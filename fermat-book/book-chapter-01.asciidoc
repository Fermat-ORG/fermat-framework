...
As a Master Plan, Fermat has it's own vision of how a collaborative project should be carried out. The main concept is to define a large master plan far into the future, and start building it phase by phase adjusing the target when needed. This approach allows the optimization on the use of available resources as instead of competing between each other they are collaborating to produce the best possible resoult between all of them.
