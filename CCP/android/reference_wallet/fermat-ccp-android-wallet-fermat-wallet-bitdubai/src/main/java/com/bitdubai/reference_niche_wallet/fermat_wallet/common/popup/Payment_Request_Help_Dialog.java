 ...
 */
public class Payment_Request_Help_Dialog extends FermatDialog<ReferenceAppFermatSession<FermatWallet>,ResourceProviderManager> implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
...
//        if(checkbox_not_show.isChecked()){
        ...
        try {
            ...
            FermatWalletSettings fermatWalletSettings = getSession().getModuleManager().loadAndGetSettings(getSession().getAppPublicKey());
            if(fermatWalletSettings!=null) {
                fermatWalletSettings.setIsPaymentHelpEnabled(!checkbox_not_show.isChecked());
                getSession().getModuleManager().persistSettings(getSession().getAppPublicKey(), fermatWalletSettings);
            }else{
                Log.e(TAG, "FermatWalletSettings null");
            }
