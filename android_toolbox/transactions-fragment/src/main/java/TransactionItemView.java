...
public class TransactionItemView
    ...
    public static class Builder {
        ...
        private TransactionItemView transactionItemView;
        ...
        // Icons
        private int completeDrawable = -1;
        private int pendingDrawable = -1;
        ...
        public Builder(Context context) {
            transactionItemView = new TransactionItemView(context);
        }

        public TransactionItemView build() {
            // To
            if(toText != -1)
                transactionItemView.setText(toText, Views.toTextView);
            if(toTextSize != -1)
                transactionItemView.setTextSize(toTextSize, Views.toTextView);
            if(toTextColor != -1)
                transactionItemView.setTextColor(toTextColor, Views.toTextView);
            if(toValueTextSize != -1)
                transactionItemView.setTextSize(toValueTextSize, Views.toValueTextView);
            if(toValueTextColor != -1)
                transactionItemView.setTextColor(toValueTextColor, Views.toValueTextView);

            // From
            if(fromText != -1)
                transactionItemView.setText(fromText, Views.fromTextView);
            if(fromTextSize != -1)
                transactionItemView.setTextSize(fromTextSize, Views.fromTextView);
            if(fromTextColor != -1)
                transactionItemView.setTextColor(fromTextColor, Views.fromTextView);
            if(fromValueTextSize != -1)
                transactionItemView.setTextSize(fromValueTextSize, Views.fromValueTextView);
            if(fromValueTextColor != -1)
                transactionItemView.setTextColor(fromValueTextColor, Views.fromValueTextView);

            // Date
            if(dateTextSize != -1)
                transactionItemView.setTextSize(dateTextSize, Views.dateTextView);
            if(dateTextColor != -1)
                transactionItemView.setTextColor(dateTextColor, Views.dateTextView);

            // State
            if(stateTextSize != -1)
                transactionItemView.setTextSize(stateTextSize, Views.stateTextView);
            if(stateTextColor != -1)
                transactionItemView.setTextColor(stateTextColor, Views.stateTextView);

            // Note
            if(noteTextSize  != -1)
                transactionItemView.setTextSize(noteTextSize , Views.noteTextView);
            if(noteTextColor != -1)
                transactionItemView.setTextColor(noteTextColor, Views.noteTextView);

            // Amount
            if(amountTextSize != -1)
                transactionItemView.setTextSize(noteTextSize, Views.amountTextView);
            if(amountTextColor != -1)
                transactionItemView.setTextColor(noteTextColor, Views.amountTextView);

            // Background
            if(backgroundColor != -1)
                transactionItemView.setBackgroundColor(backgroundColor);

            if(backgroundDrawable != -1)
                transactionItemView.setDrawable(backgroundDrawable);

            // Icon
            if(completeDrawable != -1 && pendingDrawable != -1)
                transactionItemView.setStateImageViewDrawables(completeDrawable, pendingDrawable);

            // Ripple
            if(rippleColor != -1)
                transactionItemView.setRippleColor(rippleColor);

            return  transactionItemView.buildView();
        }

        // To
        public Builder setToText(int text) {
            toText = text;
            return this;
        }

        public Builder setToTextSize(int size) {
            toTextSize = size;
            return this;
        }

        public Builder setToTextColor(int color) {
            toTextColor = color;
            return this;
        }

        public Builder setToValueTextSize(int size) {
            toValueTextSize = size;
            return this;
        }

        public Builder setToValueTextColor(int color) {
            toValueTextSize = color;
            return this;
        }


        // From
        public Builder setFromText(int text) {
            fromText = text;
            return this;
        }

        public Builder setFromTextSize(int size) {
            fromTextSize = size;
            return this;
        }

        public Builder setFromTextColor(int color) {
            fromTextColor = color;
            return this;
        }

        public Builder setFromValueTextSize(int size) {
            fromValueTextSize = size;
            return this;
        }

        public Builder setFromValueTextColor(int color) {
            fromValueTextSize = color;
            return this;
        }


        // Date
        public Builder setDateTextSize(int size) {
            dateTextSize = size;
            return this;
        }

        public Builder setDateTextColor(int color) {
            dateTextColor = color;
            return this;
        }
        ...
        // State
        public Builder setStateTextSize(int size) {
            stateTextSize = size;
            return this;
        }

        public Builder setStateTextColor(int color) {
        ...
        // Note
        public Builder setNoteTextSize(int size) {
            noteTextSize = size;
            return this;
        }

        public Builder setNoteTextColor(int color) {
            noteTextColor = color;
            return this;
        }
        ...
        // Amount
        public Builder setAmountTextSize(int size) {
            amountTextSize = size;
            return this;
        }

        /*public Builder setAmountTextColor(int color) {
        ...
        // Icons
        public Builder setStateDrawables(int complete, int pending) {
            completeDrawable = complete;
            pendingDrawable = pending;
            return this;
        }
    }
}
