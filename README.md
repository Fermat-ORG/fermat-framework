...
Fermat is a master plan to avoid getting to that future through waiting for independent entrepreneurs to build each of these wallets one at the time, repeating every time all the math, accounting, cryptography and algorithms that are common to all these wallets. The Fermat system is a master platform that handles all that and by doing  the heavy lifting enables any developer in the world to easily create niche wallets on top of its shared infraestructure. Our concept of wallet is broad and includes any financial application that handles digital money or digital assets.
