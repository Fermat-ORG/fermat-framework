...
1. Check that N >= 0, if not we end the execution with an error fermatMessage
2. Check that the available amount >= N, if not end the execution with
an error fermatMessage
...
will work with only one fermatPacketType of crypto currency from the beginning of it
...
1. Check that N >= 0, if not we end the execution with an error fermatMessage
2. Check that the available amount >= N, if not end the execution with an error fermatMessage
