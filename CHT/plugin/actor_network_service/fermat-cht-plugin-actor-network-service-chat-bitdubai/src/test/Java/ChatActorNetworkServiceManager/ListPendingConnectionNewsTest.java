...
public class ListPendingConnectionNewsTest {
    ...
    @Test
    public void listPendingConnectionNews() throws CantListPendingConnectionRequestsException {
        ChatActorNetworkServiceManager chatActorNetworkServiceManager = mock(ChatActorNetworkServiceManager.class);

        when(chatActorNetworkServiceManager.listPendingConnectionNews(Mockito.any(Actors.class))).thenReturn(new java.util.List<ChatConnectionRequest>() {
            @Override
            public int size() {
                return 0;
            }
            ...
            @Override
            public java.util.List<ChatConnectionRequest> subList(int fromIndex, int toIndex) {
                return null;
            }
        }).thenCallRealMethod();
        assertThat(chatActorNetworkServiceManager.listPendingConnectionNews(Mockito.any(Actors.class))).isNotNull();
    }
