...
import com.bitdubai.fermat_api.layer._12_middleware.wallet.exceptions.DebitFailedException;
...
class MiddlewareDebitTransaction  {
    ...
    void debit(FiatAccount fiatAccount, long fiatAmount, CryptoAccount cryptoAccount, long cryptoAmount) throws DebitFailedException {
        ...
        if ( inMemoryFiatAccount == null ) {
            throw new DebitFailedException(DebitFailedReasons.FIAT_ACCOUNT_DOES_NOT_BELONG_TO_THIS_WALLET);
        }
        ...
        if ( inMemoryCryptoAccount == null ) {
            throw new DebitFailedException(DebitFailedReasons.CRYPTO_ACCOUNT_DOES_NOT_BELONG_TO_THIS_WALLET);
        }
        ...
        if (inMemoryFiatAccount.getStatus() == AccountStatus.LOCKED) {
            throw new DebitFailedException(DebitFailedReasons.FIAT_ACCOUNT_ALREADY_LOCKED);
        }

        if (inMemoryCryptoAccount.getStatus() == AccountStatus.LOCKED) {
            throw new DebitFailedException(DebitFailedReasons.CRYPTO_ACCOUNT_ALREADY_LOCKED);
        }
        ...
        if (inMemoryFiatAccount.getStatus() != AccountStatus.OPEN) {
            throw new DebitFailedException(DebitFailedReasons.FIAT_ACCOUNT_NOT_OPEN);
        }
