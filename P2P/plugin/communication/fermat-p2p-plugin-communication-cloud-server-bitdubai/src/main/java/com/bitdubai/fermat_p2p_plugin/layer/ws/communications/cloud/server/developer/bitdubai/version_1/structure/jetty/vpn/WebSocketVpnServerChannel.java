...
public class WebSocketVpnServerChannel {
     ...
     */
    private Map<String, VpnClientConnection> participantsConnections;
    ...
    public void onWebSocketConnect(Session session) throws IOException {
        ...
        if (temp_i != null && temp_i != ""){
            ...
            this.vpnClientConnection = new VpnClientConnection(session);
             ...
             */
            clientIdentity   =  contentJsonObject.get(JsonAttNamesConstants.CLIENT_IDENTITY).getAsString();
            ...
            PlatformComponentProfile participant = new PlatformComponentProfileCommunication().fromJson(contentJsonObject.get(JsonAttNamesConstants.APPLICANT_PARTICIPANT_VPN).getAsString());

            /*
             * Get the client identity and the participant profile
             */
            vpnClientConnection.setVpnClientIdentity(vpnClientIdentity);
            vpnClientConnection.setPlatformComponentProfile(participant);

            participantsConnections.put(session.getId(), vpnClientConnection);

            if (participantPendingToReconnect != null) {

                LOG.info("All participant are connected = " + (Integer.compare(session.getOpenSessions().size(), 2) == 0));

                //Validate if all participantsConnections register are connect
                if((Integer.compare(session.getOpenSessions().size(), 2) == 0)){

                    List<VpnClientConnection> participants = (List<VpnClientConnection>) participantsConnections.values();

                    PlatformComponentProfile peer1 = participants.get(0).getPlatformComponentProfile();
                    PlatformComponentProfile peer2 = participants.get((participants.size()-1)).getPlatformComponentProfile();
    ...
    public void onWebSocketText(String fermatPacketEncode){
        ...
        if (receiveFermatPacket.getFermatPacketType() == FermatPacketType.MESSAGE_TRANSMIT){
            ...
            */
             ...
             */
            VpnClientConnection clientConnectionDestination = participantsConnections.get(fermatMessage.getReceiver());
            ...
            if (clientConnectionDestination != null && clientConnectionDestination.getSession().isOpen()){
                ...
                LOG.info("Sending msg to: " + clientConnectionDestination.getPlatformComponentProfile().getAlias());
    ...
    private void sendNotificationPacketReconnected(VpnClientConnection vpnClientConnection){
        ...
        Iterator<VpnClientConnection> iterator = participantsConnections.values().iterator();
        while (iterator.hasNext()){
            ...
            if (connection.getSession().getId() != vpnClientConnection.getSession().getId()){
                ...
                Gson gson = new Gson();
                JsonObject packetContent = new JsonObject();
                packetContent.addProperty(JsonAttNamesConstants.REMOTE_PARTICIPANT_VPN, vpnClientConnection.getPlatformComponentProfile().getIdentityPublicKey());
                packetContent.addProperty(JsonAttNamesConstants.NETWORK_SERVICE_TYPE, networkServiceType.toString());
    ...
    /**
     * Get the vpnServerIdentity value
     *
     * @return vpnServerIdentity current value
     */
    public ECCKeyPair getVpnServerIdentity() {
        return vpnServerIdentity;
    }

    @Override
    public String toString() {
        return "WebSocketVpnServerChannel{" +
                "vpnServerIdentity=" + vpnServerIdentity.getPrivateKey() +
                '}';
    }
}
