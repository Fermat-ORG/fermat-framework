...
/**
 * The Class <code>com.bitdubai.fermat_dmp_plugin.layer.wallet_module.crypto_wallet.developer.bitdubai.version_1.structure.WalletModuleCryptoCrypto</code>
 ...
 * @since Java JDK 1.7
 */
public class WalletModuleCryptoCrypto implements CryptoWallet, DealsWithBitcoinWallet, DealsWithCryptoVault, DealsWithErrors, DealsWithExtraUsers, DealsWithOutgoingExtraUser, DealsWithWalletContacts, DealsWithCryptoAddressBook {
        ...
        try {
            ...
            for(WalletContactRecord r : walletContactsRegistry.listWalletContacts(walletPublicKey)){
                ...
                switch (r.getActorType()) {
                    case EXTRA_USER:
                        image = extraUserManager.getPhoto(r.getActorPublicKey());
                        break;
                    default:
    ...
    public CryptoWalletWalletContact createWalletContact(CryptoAddress receivedCryptoAddress,
        ...
        try{
            ...
            if (walletContactRecord == null)
                return new CryptoWalletWalletModuleWalletContact(walletContactsRegistry.createWalletContact(actorPublicKey, actorName, actorType, receivedCryptoAddress, walletPublicKey));
    ...
    @Override
    public void updateContactPhoto(String actorPublicKey, Actors actor, byte[] photo) throws com.bitdubai.fermat_api.layer.dmp_wallet_module.crypto_wallet.exceptions.CantUpdateWalletContactException {
        switch (actor) {
            case EXTRA_USER:
    ...
    public CryptoWalletWalletContact findWalletContactById(UUID contactId) throws CantFindWalletContactException, com.bitdubai.fermat_api.layer.dmp_wallet_module.crypto_wallet.exceptions.WalletContactNotFoundException {
        try {
            ...
            switch (walletContactRecord.getActorType()) {
                case EXTRA_USER:
                    image = extraUserManager.getPhoto(walletContactRecord.getActorPublicKey());
                    break;
                default:
                    throw new CantGetAllWalletContactsException("UNEXPECTED ACTOR TYPE",null,"","incomplete switch");
            }
            ...
            switch (bitcoinWalletTransaction.getTransactionType()) {
                ...
                case DEBIT:
                    try {
                        ...
                        WalletContactRecord walletContactRecord = walletContactsRegistry.getWalletContactByActorPublicKey(bitcoinWalletTransaction.getActorToPublicKey());
                        if (walletContactRecord != null)
    ...
    private Actor getActorByActorPublicKeyAndType(String actorPublicKey, Actors actorType) {
        switch (actorType) {
            case EXTRA_USER:
                return extraUserManager.getActor(actorPublicKey);
            default:
                return null;
        }
